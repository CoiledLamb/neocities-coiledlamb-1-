(() => {
  // Global error reporter — writes to #lab-status so boot failures are visible
  window.addEventListener('error', (e) => {
    const el = document.getElementById('lab-status');
    if (el) el.textContent = `JS ERROR:\n${e.message}\n${e.filename?.split('/').pop()}:${e.lineno}`;
    console.error('[OilSpill] uncaught error:', e);
  });
  ...
  S.debugStats = {
    frame: 0, updated: 0, drawn: 0,
    invalidVelocity: 0, invalidPosition: 0,
  ...
  function buildGrid() {
    ...
    S.grid = Array.from({ length: rows }, () => Array.from({ length: cols }, () => []));
  ...
  function animate() {
    S.debugStats.frame++;
    S.debugStats.updated = S.debugStats.drawn = 0;
    S.debugStats.invalidVelocity = S.debugStats.invalidPosition = 0;
    S.debugStats.typeCounts.blue = S.debugStats.typeCounts.teal = S.debugStats.typeCounts.purple = 0;
    ...
    if (!isPaused) {
      ...
      for (let i = 0; i < S.dots.length; i++) {
        ...
        d.draw();
        S.debugStats.drawn++;
        if (S.debugStats.typeCounts[d.type] !== undefined) S.debugStats.typeCounts[d.type]++;
      }
    }
