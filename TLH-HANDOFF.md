...
## ✅ CURRENT STATE: v0.0.9.5 shipped (+ .5.1-.5.4 polish) + v0.0.9.6 commits 1-6 (terrain bones, label normalization, shortcut rewrite + severe trips, ladder + anchor gear, world-overlay + pkg-origin schema v9 + interior spawns, trample + pass carving) + .6.1/.6.2 polish passes; v0.0.9.6 commit 7 of 7 (storm sweep + multi-concurrent + trample sync) in flight, then v0.0.9.6.9 sim harness
...
7. **Storm sweep + multi-concurrent + spawn 2× + trample multiplayer sync.** Interior storm travel, multiple storms, combine/intersect stub, spatial wear accelerator. PLUS commit-6 deferral: trample milestone broadcast sync (at 0.25 / 0.5 / 0.85 crossings per cell — threshold-gated to bound the broadcast volume). Verifiable: observe multiple concurrent storms across ring + interior; placed ladder inside storm radius wears faster; peer trample crossings arrive + merge locally taking max value (emergent paving works across players).
