...
_last updated: 2026-04-18 (**v0.0.9.6 commit 1 of 7 landed**: terrain bones + plan corrections. [js/data/terrain.js](js/data/terrain.js) deterministic `terrainAt(x, y)` classifier anchored at corner + rim NPCs; all 7 biomes (river / clay-bed / mountain / rocky-hills / plateau / desert / flat) render cosmetically on the 2D map via [drawInterior in js/render/route-map.js](js/render/route-map.js). River geometry corrected to theta (NE) → delta (bottom-rim, dammed) with 2 W tributaries; delta-dam story cleaned (catches theta river, not mountain runoff); plateau hardcoded-destination restriction dropped in favor of retuned dest-weight curve (commit 2). Prior shipped: v0.0.9.5 + .5.1 + .5.2 (trust-rewards-via-shop) + .5.3 (tau upgrade swap) + .5.4 (last log prompts → auto). Remaining v0.0.9.6 commits: 2 dest-weight retune + label pools, 3 shortcut rewrite + interior on-grid, 4 ladder + anchor gear, 5 world-overlay + pkg-origin schema + interior spawns, 6 placement + trails + pass carving, 7 storm sweep + multi-concurrent. See [v0.0.9.6 implementation plan](#v0096-implementation-plan).)_
...
## ✅ CURRENT STATE: v0.0.9.5 shipped (+ .5.1-.5.4 polish); v0.0.9.6 commit 1 of 7 in flight
...
2. **Dest-weight retune + niche-label expansion + interior-origin label pools.** Retune the 40/25/15/10/5/5 curve for the 12-node ring preserving forward-bias shape. Extend niche-label pools for new thin-dest pairs opened up by the .5 roster. Add **terrain-origin label pools** for cell-native interior spawns (plateau ruins → rho / mountain salvage → tau / rocky-hills spillage → iota, 3-5 labels per pocket). Verifiable: spawn distribution histograms across many pkgs confirm curve shape; niche labels fire for new pairs.
3. **Shortcut mechanic rewrite + interior on-grid + pickup scanning active.** Retire flat multipliers, wire per-cell terrain effects, flip gates for weather + scanner + river-refill + pickup scanning. Verifiable: shortcut through various terrain, observe per-cell effects; pkg scan works in interior.
4. **Ladder + anchor gear.** Kit-bar integration (existing kit bar gains 2 new slot types), shop entries at 5c each, durability math (12h wall-clock), wear visual states, directional penalty mitigation. Verifiable: buy gear, cross mountain, observe penalty changes.
5. **World-overlay system + pkg-origin schema extension + interior pkg spawning.** Shared data plumbing, canonical IDs, wall-clock timestamps, multiplayer sync, baked upgrade flags. Pkg data model gains `origin.type = 'terrain' | 'npc'`. Cell-native interior spawns on plateau tops + mountain + rocky-hills cells. **Save schema v8 → v9 lands here.** Verifiable: cross-session persistence, two sessions see same objects; interior pkgs visible on map.
6. **Placement mechanic + trails + pass carving.** Drag-to-place ladders/anchors, trample-per-step, visual ramps, pass-carving at trample ≥ 0.85. Verifiable: place a ladder, reload, ladder still there; walk the same path many times, observe trample ramp.
7. **Storm sweep + multi-concurrent + spawn 2×.** Interior storm travel, multiple storms, combine/intersect stub, spatial wear accelerator. Verifiable: observe multiple concurrent storms across ring + interior; placed ladder inside storm radius wears faster.
