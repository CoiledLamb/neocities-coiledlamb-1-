...
_last updated: 2026-04-17 (**v0.0.9.5 shipped** in 5 commits: rim reshape + save migration v7→v8 (12-node rounded-square rim, player start at tau), 6 new NPCs with full dialogue + character-expressive trust profiles for all 12 (retiring default/careful/scavenger), battery baseline with innate solar trickle + stickyGun unhook, 13 new trust-reward upgrades + `efficientConsumption` migration + gear-shaped rename pass, 232-label pool expansion. Prior shipped: v0.0.9.1 + .2 + .3 + .3.1 + .4 + .4.1. Next up: **v0.0.9.6 — world patch: terrain bones + gear + world-overlay + trails**. Idle-game-first philosophy locked; all gear is placed infrastructure (12h/24h durability, 5c pricing, chained ladders as atomic multi-cell infrastructure, fall-and-ride-downstream failure mode on no-gear river crossings); plateau terrain as gear-teaching rung; ceramic wrap (theta t40) waterproofs fragile cargo. See [v0.0.9.6 implementation plan](#v0096-implementation-plan). Dispatch log virt still benched at v0.0.9.8; polish pass at v0.0.9.9+.)_
...
## ✅ CURRENT STATE: v0.0.9.3 + .3.1 + .4 + .4.1 + .5 shipped; v0.0.9.6 queued
