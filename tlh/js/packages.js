...
} from './data/packages.js?v=097-0-13';
import { cellKeyFromCoords, snapInteriorCell, mesaOutcropAt, terrainAt, MESA_OUTCROP_CENTERS } from './data/terrain.js?v=097-0-13';
import { placedGearAt, autoPlaceForCell, visiblePlacedGear } from './gear.js?v=097-0-13';
import { esc } from './util.js?v=097-0-13';
import { emit as tEmit, accum as tAccum } from './telemetry.js?v=097-0-13';
import { postActivity, shortPorterId, postLostDrop, formatPorterFreshness } from './multiplayer.js?v=097-0-13';
...
import { addTrust } from './trust.js?v=097-0-13';
import { cartFits, pushToCart } from './carrier.js?v=097-0-13';
...
import { sandalCap, renderBoots } from './boots.js?v=097-0-13';
import { addLog } from './render/log.js?v=097-0-13';
import { noteFound as cargoNoteFound } from './render/cargo-log.js?v=097-0-13';
import { renderCourierStack, renderCargoSlots } from './render/hud.js?v=097-0-13';
import { courierXY } from './segment.js?v=097-0-13';
import { pointInRing, distanceKmToNode } from './render/route-map.js?v=097-0-13';
...
// `import * as Pkg from './packages.js?v=097-0-13'` consumers keep working.
...
// ============================================================
// PKG TOOLTIP FORMATTERS
// ============================================================
// Consumed by render/hud.js (cargo grid + fieldstrip), world.js
// (ground pkg tooltips), and packages-delivery.js (none today, but
// structured for easy migration to render/pkg-tooltip.js if the
// render layer grows). Kept in packages.js because packages.js is
// already the common dependency of all three callers — moving them
// would add a new dependency line without functional gain.
// ============================================================
...
export function formatPkgTooltip(pkg) {
  ...
  if (pkg.isRecovery && pkg.recoveryFromPorter) {
    // v0.0.9.6.10.23 — freshness suffix when porterLastSeen has data.
    const fresh = formatPorterFreshness(pkg.recoveryFromPorter);
    lines.push(`[recovery from ${shortPorterId(pkg.recoveryFromPorter)}${fresh ? ' ' + fresh : ''}]`);
  } else if (pkg.isLost) {
    lines.push(`[lost]`);
  }
...
export function formatPkgTooltipHTML(pkg) {
  ...
  if (pkg.isRecovery && pkg.recoveryFromPorter) {
    // v0.0.9.6.10.23 — freshness suffix when porterLastSeen has data.
    const fresh = formatPorterFreshness(pkg.recoveryFromPorter);
    lines.push(`<div>[recovery from ${esc(shortPorterId(pkg.recoveryFromPorter))}${fresh ? ' ' + esc(fresh) : ''}]</div>`);
  } else if (pkg.isLost) {
...
export function scanForPickup() {
  ...
  for (let offset = 0; offset <= range; offset++) {
    ...
    if (cell.sandal) {
      ...
      S.sandalweedCount++;
      addLog(`harvested <span class="log-hi">sandalweed</span> <span class="log-count">(${S.sandalweedCount}/${sandalCap()})</span>`);
      renderBoots();
      continue;
    }
...
// `import * as Pkg from './packages.js?v=097-0-13'` consumers keep working.
// ============================================================
export { tryDeliver } from './packages-delivery.js?v=097-0-13';
