...
_last updated: 2026-05-03 (**v0.0.9.7.10 — interior mesa climbing + placed-gear tooltips.** Adds INTERIOR_MESAS to terrain.js mirroring topo-map.js NAT_MESAS (3 NW desert mesas at the same coords + radii topo paints). Cells inside those classify as `'plateau'` so a courier crossing them via interior shortcut engages climbing — completes the design intent ("interior mesas are climbable, just don't pull from the ring"). NOT added to mesaOutcropAt, so ring travel near them stays uninterrupted. Placed-gear (ladder + anchor) get rich tooltips on hover via the existing rich-tooltip portal: `type · tier / decays in Xh Ym`, refreshed every 500ms while open. New fmtDuration helper for wall-clock h/m/s formatting. Cache-bust 097-0-10. Kitchen design sketch still queued for v0.0.9.8 — see [v0.0.9.8 design sketch — the kitchen](#v0098-design-sketch--the-kitchen-in-flight-2026-05-03).)_
...
## just shipped — v0.0.9.7.10 — interior mesa climbing + placed-gear tooltips

Two follow-on items to .7.9, both flagged by user. Cache-bust 097-0-9 → 097-0-10 across 43 files; subtitle now reads `v0.0.9.7.10`.

### interior mesas now classify as plateau (climbable via shortcut)

Topo's NW desert mesas (`NAT_MESAS` at [topo-map.js:241](tlh/js/data/topo-map.js:241)) were visible in the heightmap but invisible to the gameplay classifier — `terrainAt` returned `'desert'` for those cells. A courier crossing them via shortcut couldn't engage the climbing mechanic.

Fix: new `INTERIOR_MESAS` constant in [terrain.js](tlh/js/data/terrain.js) mirrors topo's positions + radii (within the same r value topo uses for the smoothstep falloff so visible footprint ≈ classified footprint). Inserted into `terrainAt` BEFORE the desert check so cells inside an interior mesa's radius classify as `'plateau'` even when sitting inside nu's desert radius.

**Critically: `mesaOutcropAt` is NOT updated** — interior mesas only show up in `terrainAt`. Per the .7.9 design lock, interior mesas should NOT pull the porter from the ring; they should only engage when courier crosses them while on the interior. The .7.9 trample/gear gate (`onInterior || onMesaOutcrop`) handles this naturally — the `onInterior` branch fires for shortcuts, and interior mesa cells don't trigger the `onMesaOutcrop` check.

Note on stream/mesa overlap: topo's stream paths cut through one of the NW mesas (mesa 3 at 55, 140 overlaps the upper-W feeder stream). The river/clay check happens first in `terrainAt`, so cells where the stream crosses the mesa classify as river/clayBed, not plateau — matches the visual heightmap, which renders the river carve (-0.32 elevation) overriding the mesa stamp (+0.10). Player sees a stream cutting through the mesa; the courier's classification matches what's painted.

### placed-gear tooltips

Existing native SVG `<title>` on placed gear gave a basic browser tooltip (`ladder — fresh`) but nothing about decay timing. Player had no in-game feedback on how long until a placed ladder/anchor breaks.

Added rich-tooltip wiring on hover:
- **Render**: each `route-gear` `<text>` element gains `data-gear-x` + `data-gear-y` attributes (snapped cell coords) so the mousemove handler can look up the entry.
- **Hover**: `bindRouteInteractions` mousemove detects `route-gear` class, calls `placedGearAt(x, y)` to fetch the entry, and shows a rich tooltip with `type · tier` head + `decays in Xh Ym` line, refreshed every 500ms via the rich-tooltip refresher hook.
- **State**: new `S._transient.hoveredGearKey` tracks the active gear hover symmetrically with `hoveredNodeId`. Cleared on mouseleave or when hover transitions to a node / empty space, so the refresher doesn't tick on stale state.
- **Helper**: new `fmtDuration(ms)` formats wall-clock duration as `Xs` / `Xm` / `Xh Ym`. Distinct from `fmtEta` which uses `~` prefix for estimate-y values.

Native `<title>` element **removed** — it would trigger the browser's native tooltip on hover after a delay, conflicting with the rich tooltip. Replaced with `aria-label` on the `<text>` element, which screen readers announce without producing a competing native popup.

Verified in browser preview: 32 visible gear elements rendered with `data-gear-x/y` attrs (out of 55 total in long-running save — density cap working). Sample fresh ladder produced `ladder · fresh / decays in 11h 15m` from a 12h-base lifetime placed ~44 min ago.

### file map

**Modified:**
- [tlh/js/data/terrain.js](tlh/js/data/terrain.js) — `INTERIOR_MESAS` + `INTERIOR_MESA_CENTERS` export; `terrainAt` extended with the new check
- [tlh/js/render/route-map.js](tlh/js/render/route-map.js) — `placedGearAt` import added; `fmtDuration` helper added; gear render gets `data-gear-x/y`; mousemove + mouseleave extended for gear hover
- [tlh/js/state.js](tlh/js/state.js) — `_transient.hoveredGearKey` slot added
- [tlh/the-long-haul.html](tlh/the-long-haul.html) — subtitle bump
- 43 files — cache-bust `097-0-9` → `097-0-10`

### explicit non-scope (decisions logged)

- **Interior mesa scope** — kept to topo's existing NAT_MESAS (3 NW desert). New ring-rim mesa stamps NOT added; if added later, must use NAT_MESAS smoothstep pattern (per user note, "easy to mess up — match how our heightmap does elevation for them").
- **mesaOutcropAt update** — explicitly left alone. Interior mesas must NOT pull from the ring. Only ring teaching outcrops do.
- **Stream/mesa overlap** — accepted. River carve wins over mesa stamp visually + gameplay-wise. Matches topo behavior.
- **Tooltip placement** — `cursor` placement (mouse-anchored, not target-anchored) so the tooltip tracks the mouse. Same pattern as the route-node tooltip.
...
## just shipped — v0.0.9.7.9 — porter ring-corner fix + topo stream alignment

Two-part patch landed before opening the kitchen (.8). Cache-bust 097-0-8 → 097-0-9 across 43 files; subtitle now reads `v0.0.9.7.9`.

### the long-running-save bug

User flagged via screenshot: in their long-running save, a visible "trail into the interior" had accumulated near the city ruins (xi) corner, looking like the courier was deviating off the ring path. Root cause traced to two coupled mechanics in [tlh/js/main.js:330-360](tlh/js/main.js:330):

1. **Trample accumulation** ([line 347 pre-fix](tlh/js/main.js:347)): `if (onInterior || currentTerrain === 'plateau') { addTrampleAt(...) }`. The `'plateau'` branch was added in v0.0.9.6.9.3 to support the 4 ring-rim teaching outcrops (mesas placed on H↔A↔ν↔·↔B midpoints) — courier walking the ring through those classify as `'plateau'` and accumulate trample, which is the design intent.
2. **Auto-ladder placement** ([line 337 pre-fix](tlh/js/main.js:337)): `if (GEAR_FOR_TERRAIN[currentTerrain] && courierXYNow) { autoPlaceForCell(...) }`. Same logic, same gate behavior. Walking through plateau cells auto-places ladders.

Unintended side effect: xi's corner plateau (`PLATEAU_R = 55` from xi at SE corner per [terrain.js:202](tlh/js/data/terrain.js:202)) extends INWARD from the SE corner into the ring polygon. Ring segments gamma↔xi↔delta walk along the corner, and the courier's `(x, y)` falls within xi's plateau radius for cells that are also INSIDE the ring polygon. Both trample and ladder placement fire on those cells. Over many laps, trampled cells become visibly painted as a trail looping inward of the ring near the SE corner.

### the design lock

Three rounds of clarification with user landed on:

> **Porter doesn't seek mesas while on the ring.** If they cross one in the interior (shortcut) or on a deliberate teaching outcrop (.9.6.9.3 placement on the ring midpoints), they engage it. Otherwise, ring travel is just ring travel — no incidental engagement with corner plateaus.

So the gate must distinguish:
- **Interior cells** → engage everything (trample + ladders)
- **Ring + mesa outcrop** → engage everything (deliberate teaching design)
- **Ring + non-mesa plateau** (e.g., xi's corner radius) → engage NOTHING

### the fix

Both the trample bump and the gear auto-place gated to `onInterior || onMesaOutcrop`, where `onMesaOutcrop = currentTerrain === 'plateau' && mesaOutcropAt(x, y)`. Imports `mesaOutcropAt` from terrain.js into main.js. Adds an explicit `ringSeekingPlateau` branch for clarity.
...
// gear auto-place: skip when ring is incidentally crossing corner plateau
if (GEAR_FOR_TERRAIN[currentTerrain] && courierXYNow && !ringSeekingPlateau) { ... }
// trample: same gate
if (onInterior || onMesaOutcrop) { addTrampleAt(...); ... }
```

Verified in browser preview:
- xi corner cell (305, 305) classifies as `'plateau'` ✓ (unchanged)
- `mesaOutcropAt(305, 305)` returns `null` ✓ (xi corner is NOT a mesa outcrop)
- `mesaOutcropAt(84, 132)` returns `'A-nu mesa'` ✓ (teaching outcrops still detected)
- Net: ring near xi corner no longer triggers trample/gear; teaching outcrops still do.

**Existing trampled cells in long-running saves stay visible** — no migration. Per user direction, trample decay (stubbed in [trail.js:22](tlh/js/trail.js:22)) will absorb them in a future patch when decay lands. New play won't add to the historical trail.

### topo stream alignment

[terrain.js TRIBUTARIES](tlh/js/data/terrain.js) updated from 2 hand-placed line segments to **14 short segments matching topo-map.js's `STREAM_CTRLS`**. Topo has 5 streams drawn as Catmull-Rom curves; the 5th goes off-map (y > 400) so it's skipped. The other 4 are sampled into 3-4 straight segments each, close enough to the underlying curve that `distToSegment` within `RIVER_HALF_W` (7) + `CLAY_BANK_EXTRA` (5) hits the right cells.

Verified: sample points along all 4 streams classify as `'river'`. Visible streams in topo now match gameplay river/clayBed cells.

### topo mesas — explicitly not changed

Topo's 3 NW desert mesas (`NAT_MESAS` at [topo-map.js:241](tlh/js/data/topo-map.js:241)) left as-is — interior visual scenery only, no porter-seeking. Per design lock, this is fine: porter won't path toward them, but if a shortcut crosses one, the courier engages it. terrain.js's 4 ring-rim teaching outcrops (`MESA_OUTCROPS` at [terrain.js:103](tlh/js/data/terrain.js:103)) stay where they are — on the ring midpoints, deliberately. Two systems coexist with different roles.

### file map

**Modified:**
- [tlh/js/main.js](tlh/js/main.js) — `mesaOutcropAt` import added; ring-seeking-plateau gate added; trample + gear auto-place both gated
- [tlh/js/data/terrain.js](tlh/js/data/terrain.js) — `TRIBUTARIES` rewritten from 2 to 14 segments (topo-aligned)
- [tlh/the-long-haul.html](tlh/the-long-haul.html) — subtitle bump
- 43 files — cache-bust `097-0-8` → `097-0-9`

### explicit non-scope (decisions logged)

- **One-time trample-cleanup migration** — declined per user. Existing trampled cells in long-running saves stay until decay lands.
- **Topo mesa stamps at ring-rim positions** — declined for now. If added later, must use the existing `NAT_MESAS` smoothstep pattern rather than hand-coded coordinates (per user note: "easy to mess up — match how our heightmap does elevation for them").
- **Porter shortcut tolerances near nu** — covered by the same fix. Nu's biome is desert (not plateau), so the trample gate didn't apply to nu's surrounding ring path even before this patch. Mesa outcrops near nu (A-nu, nu-psi) trample as designed.
- **Lake biome in terrain.js** — left out. Topo's reservoir lake basin stays cosmetic-only (per design intent: dam ends there, player crosses the dam visually, not a gameplay biome).
